...
contract OptimismDomain is BridgedDomain {
    ...
    MessengerLike public constant L2_MESSENGER =
        MessengerLike(0x4200000000000000000000000000000000000007);
    ...
    function relayFromHost(bool switchToGuest) external override {
        ...
        for (; lastFromHostLogIndex < logs.length; lastFromHostLogIndex++) {
            ...
            if (
                log.topics[0] == SENT_MESSAGE_TOPIC &&
                ...
                (
                    ...
                    uint256 nonce,
                    uint256 gasLimit
                ) = abi.decode(log.data, (address, bytes, uint256, uint256));
                vm.prank(malias);
